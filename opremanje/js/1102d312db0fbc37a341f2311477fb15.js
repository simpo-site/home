...
this.$distanceInput.on('change',(event)=>{this.updateLocationData({distance:event.target.value})});this.defaultLocation=$filter.data('current-location');if(this.defaultLocation.address){this.$locationInput.val(this.defaultLocation.address)}
